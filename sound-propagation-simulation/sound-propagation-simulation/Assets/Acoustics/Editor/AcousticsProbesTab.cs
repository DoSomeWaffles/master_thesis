...
namespace Microsoft.Acoustics.Editor
{
    public class AcousticsProbesTab : ScriptableObject
    {
        ...
        {
            public AcousticMesh AcousticMesh;
            public TritonSimulationParameters SimParams;
            public TritonOperationalParameters OpParams;
            public AcousticMaterialLibrary Matlib;
        };

        /// Properties related to the probe preview
        // PreviewResults contains the voxelization and probe data from Triton
        SimulationConfig m_previewResults;
        public bool HasPreviewResults { get { return (m_previewResults != null && !m_previewResults.IsInvalid && !m_previewResults.IsClosed); } }
        public void SetPreviewResults(SimulationConfig newPreviewResults)
        {
            m_previewResults = newPreviewResults;
        }
        public void ClearPreviewResults()
        {
            m_previewResults?.Dispose();
        }
        ...
        static AcousticsMaterialsTab m_MaterialsTab;
        ...
        public void UpdateComputeTimeEstimate()
        {
            ...
            float simulationVolume = (AcousticsEditor.s_AcousticsParameters.PerProbeSimulationRegion_Upper.x - AcousticsEditor.s_AcousticsParameters.PerProbeSimulationRegion_Lower.x) * // Length
                                     (AcousticsEditor.s_AcousticsParameters.PerProbeSimulationRegion_Upper.y - AcousticsEditor.s_AcousticsParameters.PerProbeSimulationRegion_Lower.y) * // Width
                                     (AcousticsEditor.s_AcousticsParameters.PerProbeSimulationRegion_Upper.z - AcousticsEditor.s_AcousticsParameters.PerProbeSimulationRegion_Lower.z);  // Height
        ...
        public static AcousticMaterialLibrary CreateMaterialLibrary()
        {
            ...
            var materialsUnmanagedPtr = Marshal.AllocHGlobal(materialsUnmanagedSize * materialMap.Count);

            try
            {
                int i = 0;
                var acousticMaterial = new TritonAcousticMaterial();
                foreach (KeyValuePair<string, float> entry in materialMap)
                {
        ...
        public bool CalculateProbePoints(bool showDialog = true)
        {
            ...
            AcousticMaterialLibrary materialLib = CreateMaterialLibrary();
            ...
            TritonBoundingBox perProbeSimRegion = new TritonBoundingBox
            {
                MinCorner = AcousticsEditor.TritonVec3dFromVector3(AcousticsEditor.s_AcousticsParameters.PerProbeSimulationRegion_Lower),
                MaxCorner = AcousticsEditor.TritonVec3dFromVector3(AcousticsEditor.s_AcousticsParameters.PerProbeSimulationRegion_Upper)
            };
            ...
            TritonSimulationParameters simParams = new TritonSimulationParameters
            {
                ...
                ProbeSpacing = pss,
                PerProbeSimulationRegion = perProbeSimRegion
            };
        ...
        void DoCalculateProbes(object parms)
        {
            ...
            if (!SimulationConfigNativeMethods.TritonPreprocessor_SimulationConfiguration_Create(cpp.AcousticMesh, ref cpp.SimParams, ref cpp.OpParams, cpp.Matlib, StatusCallback, out simConfig))
            {
        ...
        public static void AddToTritonAcousticMesh(Transform transform, AcousticMesh acousticMesh, Vector3[] verticesIn, int[] trianglesIn, long materialCode, bool isNavMesh, bool needsTransform)
        {
            ...
            if (!needsTransform)
            {
                vertices = Array.ConvertAll<Vector3, AcousticsPALPublic.TritonVec3d>(verticesIn, vIn =>
                {
                    // NavMesh vertices are already in Unity World coordinates
                    Vector4 vInTransformed = AcousticsEditor.WorldToTriton(vIn);

                    return new AcousticsPALPublic.TritonVec3d(vInTransformed.x, vInTransformed.y, vInTransformed.z);
                });
            }
            else
            {
