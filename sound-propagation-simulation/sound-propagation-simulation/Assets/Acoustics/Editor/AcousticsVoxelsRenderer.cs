...
namespace Microsoft.Acoustics.Editor
{
    ...
    public class AcousticsVoxelsRenderer : AcousticsActualRenderer
    {
        ...
        private SimulationConfig m_previewData;
        ...
        {
            if (m_previewData.IsInvalid || m_previewData.IsClosed)
            {
                return;
            }
            ...
            if (!m_staticValuesCached)
            {
                ...
                m_boxMin = AcousticsEditor.TritonToWorld(voxelBox.MinCorner.ToVector3());
                Vector3 boxMax = AcousticsEditor.TritonToWorld(voxelBox.MaxCorner.ToVector3());
            ...
            // It's possible for the viewport to extend below the voxel min corner, which would cause underflow.
            // Clamp min values to 0 and adjust starting voxel appropriately to prevent this.
            if (xMin < 0)
            {
                startVoxelCenter.x += (-xMin * m_cellSize);
                xMin = 0;
            }
            if (yMin < 0)
            {
                startVoxelCenter.y += (-yMin * m_cellSize);
                yMin = 0;
            }
            if (zMin < 0)
            {
                startVoxelCenter.z += (-zMin * m_cellSize);
                zMin = 0;
            }
        ...
        public void SetPreviewData(SimulationConfig results)
        {
