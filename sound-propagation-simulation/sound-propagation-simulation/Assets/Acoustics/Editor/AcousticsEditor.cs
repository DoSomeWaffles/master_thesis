...
namespace Microsoft.Acoustics.Editor
{
    public class AcousticsEditor : EditorWindow, IHasCustomMenu
    {
        ...
        // ASP.Net DataProtector for encrypting/decrypting Azure creds
        private IDataProtector m_DataProtector;
        ...
        public void Initialize()
        {
            if (!m_initialized)
            {
                ...
                var provider = DataProtectionProvider.Create("Microsoft.ProjectAcoustics.Unity");
                m_DataProtector = provider.CreateProtector("Microsoft.ProjectAcoustics.Unity.EncryptDecrypt");
                ...
                if (!String.IsNullOrEmpty(azureCreds))
                {
                    ...
                    try
                    {
                        var azureCredsUnprotected = m_DataProtector.Unprotect(azureCreds);
                        EditorJsonUtility.FromJsonOverwrite(azureCredsUnprotected, s_AcousticsParameters.AzureAccounts);
                    }
                    catch (CryptographicException)
                    {
                        // Ignore failure to unprotect creds, commonly due to a new crypto provider, and re-start with fresh creds
                        Debug.Log($"Attempt to load encrypted credentials failed.  Please supply latest Azure credentials for baking.");
                    }
        ...
        static void LoadAcousticParameters()
        {
            ...
            // Overwrite the serialized value for TritonImage with the default image for this version of the runtime.
            // This is done to avoid issues when an incompatible bake toolset is retained after upgrading the plugin.
        ...
        void RenderBakeTab()
        {
            ...
            if (m_bakeCredsFoldoutOpen)
            {
                ...
                s_AcousticsParameters.AzureAccounts.BatchAccountUrl = EditorGUILayout.TextField(new GUIContent("Batch Account Endpoint", "Enter the endpoint of your Azure Batch account here."), s_AcousticsParameters.AzureAccounts.BatchAccountUrl);
                ...
                s_AcousticsParameters.AzureAccounts.StorageAccountKey = EditorGUILayout.PasswordField(new GUIContent("Storage Key", "Enter the key of the associated Azure Storage account here."), s_AcousticsParameters.AzureAccounts.StorageAccountKey);
                ...
                if (s_AcousticsParameters.TritonImage != AcousticsParameters.DefaultTritonImage)
                {
                    ...
                    s_AcousticsParameters.AzureAccounts.AzureContainerRegistryServer = EditorGUILayout.TextField(new GUIContent("Azure Container Login Server", "Enter the login server URL for the ACR hosting the docker image."), s_AcousticsParameters.AzureAccounts.AzureContainerRegistryServer);
                    s_AcousticsParameters.AzureAccounts.AzureContainerRegistryAccount = EditorGUILayout.TextField(new GUIContent("Azure Container Username", "Enter the username of the ACR hosting the docker image for authentication."), s_AcousticsParameters.AzureAccounts.AzureContainerRegistryAccount);
                    s_AcousticsParameters.AzureAccounts.AzureContainerRegistryKey = EditorGUILayout.PasswordField(new GUIContent("Azure Container Password", "Enter the password of the ACR hosting the docker image for authentication."), s_AcousticsParameters.AzureAccounts.AzureContainerRegistryKey);
                }
                else
                {
                ...
                if (!newCreds.Equals(oldCreds))
                {
                    if (anyCredsFilledIn)
                    {
                        ...
                        string newCredsProtected = m_DataProtector.Protect(newCreds);
        ...
        public static AcousticsPALPublic.TritonVec3d TritonVec3dFromVector3(Vector3 vector)
        {
            return new AcousticsPALPublic.TritonVec3d(vector.x, vector.y, vector.z);
        }
